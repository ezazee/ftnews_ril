@extends('layouts.vertical', ['title' => 'List', 'sub_title' => 'Post', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])

@section('content')
@include('sweetalert::alert')

<form action="{{ route('list.create') }}" method="GET" class="flex items-center p-4 bg-gray-100 dark:bg-gray-700 rounded-md">
    <input
        type="text"
        name="search"
        id="searchInput"
      ...
      <tbody>
        @foreach ($post as $item)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <th class="px-6 py-4">
                        <img src="{{ asset('storage/' . $item->gambar) }}" alt="{{ $item->title }}" class="w-20 h-12 object-contain rounded">
                    </th>
